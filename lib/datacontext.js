...
export const DataProvider = ({ children }) => {
  ...
  const searchRequests = async (searchText, language = 'en', filters = requestFilters) => {
    ...
    try {
      ...
      const surveysWithCategories = await Promise.all(
        filteredSurveys.map(async (item) => {
          let categoryName = { en: 'Unknown', ru: 'Неизвестно', kz: 'Белгісіз' };
          if (item.categoryId) {
            let categoryId;
      ...
      setUserSurveySearchResults(surveysWithCategories);
      const initialItems = surveysWithCategories.slice(0, userSurveysItemsPerPage);
      setPaginatedUserSurveySearchResults(initialItems);
      setCurrentUserSurveySearchPage(1);
      setUserSurveySearchError(null);
      return surveysWithCategories;
    } catch (err) {
  ...
  const fetchRequests = async (filters = requestFilters) => {
    ...
    try {
      ...
      setUserSurveys(surveysWithCategories);
      const initialItems = surveysWithCategories.slice(0, userSurveysItemsPerPage);
  ...
  useEffect(() => {
    if (user && hasProfile === true) {
      Promise.all([
        ...
        fetchPetitions(),
        fetchRequests(),
        fetchUserSurveys(), // Add initial fetch for user surveys
      ]).catch((err) => {
