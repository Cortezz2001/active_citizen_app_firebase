...
const itemsPerPage = 3;
  ...
  if (isSearchActive) {
    ...
    if (startIndex >= searchResults.length) return; // No more items to load
    ...
    // Handle regular news pagination
    const startIndex = currentNewsPage * itemsPerPage;
    if (startIndex >= news.length) return; // No more items to load
...
const updateNewsCommentCount = async (newsId) => {
  try {
    ...
    // Update news in the paginated array
    ...
    if (isSearchActive) {
      // Update main search results array
      ...
      // Update paginated search results
